...
With novalnet, you can securely store one or more payment details per shopper. This allows you to offer subscription payments, automatic top-ups to shopper accounts, and give your shoppers a faster checkout experience by using their stored card.
...
- `removeSensitiveData: false` - set this if your novalnet-integration is deployed with `removeSensitiveData: true` configuration. The reason is by default stored payment details are returned asynchronously in `additionalData` in notification and this field is being removed when `removeSensitiveData: true`.
...
https://docs.novalnet.com/online-payments/tokenization/create-and-use-tokens

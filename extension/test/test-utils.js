...
function overrideEnableHmacSignatureConfig(enableHmacSignature) {
  ...
  const updatednovalnetMerchantAccounts = new Map()
  for (const novalnetMerchantId of novalnetMerchantAccounts) {
    const novalnetConfig = config.getnovalnetConfig(novalnetMerchantId)
    novalnetConfig.enableHmacSignature = enableHmacSignature
    updatednovalnetMerchantAccounts.set(novalnetMerchantId, novalnetConfig)
  }
  config.getnovalnetConfig = function getnovalnetConfig(novalnetMerchantId) {
    return updatednovalnetMerchantAccounts.get(novalnetMerchantId)
  }
}

async function setupLocalTunnel() {
  const extensionTunnelDomain = 'ctp-novalnet-integration-tests'
...
async function ensurenovalnetWebhookForAllnovalnetAccounts(webhookUrl) {
  ...
  for (const ctpProjectKey of ctpProjectKeys) {
    ...
    for (const novalnetMerchantId of novalnetMerchantAccounts) {
      ...
      merchantIdToWebhookIdMap.set(novalnetMerchantId, webhookId)
    }
  }
}

async function setUpWebhooksAndNotificationModule() {
  const notificationTunnelDomain = 'ctp-novalnet-integration-tests-notifications'
  ...
  await ensurenovalnetWebhookForAllnovalnetAccounts(webhookUrl)
...
async function deleteWebhooks() {
  ...
  for (const novalnetMerchantId of novalnetMerchantAccounts) {
    const novalnetConfig = config.getnovalnetConfig(novalnetMerchantId)
    const webhookId = merchantIdToWebhookIdMap.get(novalnetMerchantId)
    await fetch(
      `https://management-test.novalnet.com/v1/merchants/${novalnetMerchantId}/webhooks/${webhookId}`,
      {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'X-Api-Key': novalnetConfig.apiKey,
        },
      },
    )
  }

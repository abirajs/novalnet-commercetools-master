...
describe('create-session-request-with-lineitems::execute', () => {
  ...
  let scope
  const novalnet_PERCENTAGE_MINOR_UNIT = 10000
  ...
  it(
    ...
    async () => {
      ...
      const novalnetLineItem = createSessionRequestJson.lineItems.find(
        (item) => item.id === 'test-product-sku-1',
      )
      expect(ctpLineItem.price.value.centAmount).to.equal(
        novalnetLineItem.amountIncludingTax,
      )
      expect(
        parseFloat(
          (
      ...
      ).to.equal(novalnetLineItem.amountExcludingTax)
      expect(
        ctpLineItem.price.value.centAmount -
          parseFloat(
            (
      ...
      ).to.equal(novalnetLineItem.taxAmount)
      expect(ctpLineItem.taxRate.amount * novalnet_PERCENTAGE_MINOR_UNIT).to.equal(
        novalnetLineItem.taxPercentage,
      )

      const ctpShippingInfo = ctpCart.shippingInfo
      const novalnetShippingInfo = createSessionRequestJson.lineItems.find(
        (item) => item.id === ctpShippingInfo.shippingMethodName,
      )
      expect(ctpShippingInfo.price.centAmount).to.equal(
        novalnetShippingInfo.amountIncludingTax,
      )
      expect(
        parseFloat(
          (
      ...
      ).to.equal(novalnetShippingInfo.amountExcludingTax)
      expect(
        ctpShippingInfo.price.centAmount -
          parseFloat(
            (
      ...
      ).to.equal(novalnetShippingInfo.taxAmount)
      expect(
        ctpShippingInfo.taxRate.amount * novalnet_PERCENTAGE_MINOR_UNIT,
      ).to.equal(novalnetShippingInfo.taxPercentage)
    },
  )

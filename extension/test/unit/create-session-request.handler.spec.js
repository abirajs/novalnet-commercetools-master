...
describe('create-session-request::execute::', () => {
  ...
  it('handlePayment should return the right actions', async () => {
    const novalnetGetSessionResponse = {
      amount: {
        currency: 'EUR',
        value: 1000,
      },
    ...
    scope.post('/sessions').query(true).reply(200, novalnetGetSessionResponse)
    ...
    expect(result.actions[0].fields.response).to.be.deep.equal(
      JSON.stringify(novalnetGetSessionResponse),
    )
    expect(result.actions[0].fields.response).to.be.deep.equal(
  ...
  it(
    ...
    async () => {
      ...
      expect(createSessionRequestJson.countryCode).to.equal(
        getSessionRequestWithAdditionalFields.countryCode,
      )
      expect(createSessionRequestJson.dateOfBirth).to.equal(
        getSessionRequestWithAdditionalFields.dateOfBirth,
      )
      expect(
        createSessionRequestJson.additionalData.enhancedSchemeData
          .destinationCountryCode,
      ).to.equal(
        getSessionRequestWithAdditionalFields.additionalData.enhancedSchemeData
          .destinationCountryCode,
      )
      expect(
        createSessionRequestJson.additionalData.enhancedSchemeData
      ...
      expect(
        createSessionRequestJson.billingAddress.houseNumberOrName,
      ).to.equal(ctpCartWithCustomer.billingAddress.streetNumber)
      expect(createSessionRequestJson.billingAddress.city).to.equal(
      ...
      expect(createSessionRequestJson.billingAddress.country).to.equal(
        ctpCartWithCustomer.billingAddress.country,
      )
      expect(createSessionRequestJson.shopperEmail).to.equal(
        ctpCartWithCustomer.customerEmail,
      )

      expect(createSessionRequestJson.accountInfo.accountCreationDate).to.equal(
        ctpCustomer.createdAt,
      )
      expect(createSessionRequestJson.accountInfo.accountChangeDate).to.equal(
        ctpCustomer.lastModifiedAt,
      )
      expect(createSessionRequestJson.accountInfo.accountAgeIndicator).to.equal(
        'moreThan60Days',
      )
      expect(
        createSessionRequestJson.accountInfo.accountChangeIndicator,

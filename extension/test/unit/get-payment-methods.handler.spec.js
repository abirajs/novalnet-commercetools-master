...
describe('get-payment-methods::execute::', () => {
  ...
  const novalnetCredentials = config.getnovalnetConfig(
    paymentObject.custom.fields.novalnetMerchantAccount,
  )
  ...
  it('handlePayment should return the right actions', async () => {
    const novalnetGetPaymentResponse = {
      paymentMethods: [
        {
          configuration: {
            intent: 'capture',
    ...
    nock(`${novalnetCredentials.apiBaseUrl}`)
      .post('/paymentMethods')
      .query(true)
      .reply(200, novalnetGetPaymentResponse)
    ...
    expect(result.actions[0].fields.response).to.be.deep.equal(
      JSON.stringify(novalnetGetPaymentResponse),
    )
    expect(result.actions[0].fields.response).to.be.deep.equal(

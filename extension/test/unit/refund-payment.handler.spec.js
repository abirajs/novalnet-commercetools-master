...
describe('refund-payment::execute', () => {
  ...
  beforeEach(() => {
    ...
    scope = nock(`${novalnetConfig.apiBaseUrl}/payments/8835513921644842/`)
  })
  ...
  it('when refund payment request contains reference, then it should send this reference to novalnet', async () => {
    ...
    const novalnetRequest = response.actions.find(
      (action) => action.action === 'addInterfaceInteraction',
    ).fields.request
    const novalnetRequestJson = JSON.parse(novalnetRequest)
    const requestBody = JSON.parse(novalnetRequestJson.body)
  ...
  it(
    ...
    async () => {
      ...
      const novalnetResponse = response.actions.find(
        (action) => action.action === 'addInterfaceInteraction',
      ).fields.response
      ...
      expect(novalnetResponse).to.contains('non-json-response')
    },
  )
})

...
describe('::config::', () => {
  ...
  it('when config is provided, it should load correctly', async () => {
    ...
    expect(config.default.getAllnovalnetMerchantAccounts()).to.eql([
      'novalnetMerchantAccount1',
    ])
    ...
    expect(config.default.getnovalnetConfig('novalnetMerchantAccount1')).to.eql({
      apiBaseUrl: 'apiBaseUrl',
      apiKey: 'apiKey',
      clientKey: 'clientKey',
      paypalMerchantId: '',
  ...
  it('when some values are not provided, it should provide default values', async () => {
    ...
    expect(config.default.getnovalnetConfig('novalnetMerchantAccount1')).to.eql({
      apiBaseUrl: 'https://checkout-test.novalnet.com/v71',
      apiKey: 'apiKey',
      clientKey: 'clientKey',
      paypalMerchantId: '',
    })
  ...
  it('when both config and external config file are missing, it should throw error', async () => {
    delete process.env.novalnet_INTEGRATION_CONFIG
  ...
  it('when no novalnet merchant account is provided, it should throw error', async () => {
    process.env.novalnet_INTEGRATION_CONFIG = JSON.stringify({
      commercetools: {
        ctpProjectKey1: {
          clientId: 'clientId',
    ...
    try {
      ...
      expect(e.message).to.contain('add at least one novalnet merchant account')
    }
  })
  ...
  it(
    'when extra novalnetPaymentMethodsToNames config is not provided, ' +
      'it should return default novalnetPaymentMethodsToNames config',
    async () => {
      process.env.novalnet_INTEGRATION_CONFIG = JSON.stringify({
        commercetools: {
          ctpProjectKey1: {
      ...
      expect(config.default.getnovalnetPaymentMethodsToNames()).to.eql({
  ...
  it(
    'when extra novalnetPaymentMethodsToNames config is provided, ' +
      'it should return merged novalnetPaymentMethodsToNames config',
    async () => {
      process.env.novalnet_INTEGRATION_CONFIG = JSON.stringify({
        commercetools: {
          ctpProjectKey1: {
  ...
  it('when novalnet_INTEGRATION_CONFIG is not valid JSON, it should throw error', async () => {
    process.env.novalnet_INTEGRATION_CONFIG = '{"a"}'
    try {
      await reloadModule('../../../src/config/config.js')
      expect.fail('This test should throw an error, but it did not')
    } catch (e) {
  ...
  it(
    'when novalnet_INTEGRATION_CONFIG is not set but external file is configured, ' +
    ...
    async () => {
      ...
      try {
        ...
        expect(
          loadedConfig.default.getnovalnetConfig('novalnetMerchantAccount1'),
        ).to.deep.equal({
          apiBaseUrl: 'apiBaseUrl',
          apiKey: 'apiKey',
          clientKey: 'clientKey',

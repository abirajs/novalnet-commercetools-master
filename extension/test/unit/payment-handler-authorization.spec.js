...
describe('payment-handler-authorization::execute', () => {
  ...
  const novalnetCredentials = config.getnovalnetConfig(novalnetMerchantAccount)
  ...
  beforeEach(() => {
    scope = nock(`${novalnetCredentials.apiBaseUrl}`)
  })
  ...
  it(
    'when endpoint authorization is enabled and request is authorized' +
      'then it should call /sessions on novalnet',
    async () => {
      mockCtpEnpoints._mockCtpCartsEndpoint(ctpCart, ctpProjectKey)
      scope.post('/sessions').reply(200, novalnetGetSessionResponse)
  ...
  it(
    'when endpoint authorization is enabled and request contains no authorization header value' +
      'then it should fail to call /sessions on novalnet',
    async () => {
      mockCtpEnpoints._mockCtpCartsEndpoint(ctpCart, ctpProjectKey)
      scope.post('/sessions').reply(200, novalnetGetSessionResponse)

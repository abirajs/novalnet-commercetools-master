...
export default class PaypalInitSessionFormPage extends InitSessionFormPage {
  ...
  async clickOnPaypalButton() {
    await executeInnovalnetIframe(this.page, '.paypal-button', (el) => el.click())
    await new Promise((resolve) => {
  ...
  async getPaymentAuthResult() {
    ...
    await this.page.waitForSelector('#novalnet-payment-auth-result') // make sure result has been redenered in main page

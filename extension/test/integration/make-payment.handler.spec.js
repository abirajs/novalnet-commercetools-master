...
describe('::make-payment with multiple novalnet accounts use case::', () => {
  const [commercetoolsProjectKey] = config.getAllCtpProjectKeys()
  const [novalnetMerchantAccount1, novalnetMerchantAccount2] =
    config.getAllnovalnetMerchantAccounts()
  ...
  it(
    'given a single commercetools project and payments ' +
      'when makePayment custom field is set and response from novalnet is Authorised, ' +
      'then it should connect the 2 different novalnet mechant accounts and ' +
    ...
    async () => {
      await Promise.all([
        makePayment({
          ...
          novalnetMerchantAccount: novalnetMerchantAccount1,
          metadata: {
        ...
        makePayment({
          ...
          novalnetMerchantAccount: novalnetMerchantAccount2 || novalnetMerchantAccount1,
        }),
      ])
    },
  )
  ...
  async function makePayment({ reference, novalnetMerchantAccount, metadata }) {
    const makePaymentRequestDraft = {
      amount: {
        currency: 'EUR',
        value: 1000,
    ...
    const paymentDraft = {
      ...
      paymentMethodInfo: {
        paymentInterface: constants.CTP_novalnet_INTEGRATION,
      },
      custom: {
        type: {
          typeId: 'type',

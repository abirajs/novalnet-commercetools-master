...
function createLineItems(payment, cart) {
  ...
  cart.lineItems?.forEach((item) => {
    if (item.taxRate)
      lineItems.push(_createnovalnetLineItemFromLineItem(item, locales))
  })

  cart.customLineItems?.forEach((item) => {
    if (item.taxRate)
      lineItems.push(_createnovalnetLineItemFromCustomLineItem(item, locales))
  })

  const { shippingInfo } = cart
  if (shippingInfo && shippingInfo.taxRate)
    lineItems.push(_createShippingInfonovalnetLineItem(shippingInfo, locales))
...
function _createnovalnetLineItemFromLineItem(ctpLineItem, locales) {
  ...
  return {
    ...
    taxPercentage: ctpLineItem.taxRate.amount * novalnet_PERCENTAGE_MINOR_UNIT,
  }
}

function _createnovalnetLineItemFromCustomLineItem(ctpLineItem, locales) {
  const quantity = ctpLineItem.quantity
  return {
    id: ctpLineItem.id,
    quantity: ctpLineItem.quantity,
...
function _createShippingInfonovalnetLineItem(shippingInfo, locales) {
  return {
    ...
    taxPercentage: shippingInfo.taxRate.amount * novalnet_PERCENTAGE_MINOR_UNIT,
  }
}

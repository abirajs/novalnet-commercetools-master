 ...
 * @param newState state of the transaction from the novalnet notification
...
async function getTransactionTypeAndStateOrNull(notificationRequestItem) {
  const novalnetEvents = await utils.readAndParseJsonFile(
    'resources/novalnet-events.json',
  )
  const novalnetEventCode = notificationRequestItem.eventCode
  const novalnetEventSuccess = notificationRequestItem.success

  const novalnetEvent = _.find(
    novalnetEvents,
    (e) => e.eventCode === novalnetEventCode && e.success === novalnetEventSuccess,
  )
  if (novalnetEvent && novalnetEventCode === 'CANCEL_OR_REFUND') {
    ...
    const modificationAction = notificationRequestItem.additionalData
      ...
      : null
    if (modificationAction === 'refund') novalnetEvent.transactionType = 'Refund'
    else if (modificationAction === 'cancel')
      novalnetEvent.transactionType = 'CancelAuthorization'
  }
  return (
    novalnetEvent || {
      eventCode: novalnetEventCode,
      success: novalnetEventSuccess,
...
function getSetMethodInfoNameAction(paymentMethod) {
  const paymentMethodsToLocalizedNames = config.getnovalnetPaymentMethodsToNames()

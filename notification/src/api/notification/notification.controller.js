...
import { getCtpProjectConfig, getnovalnetConfig } from '../../utils/parser.js'
...
async function handleNotification(request, response, logger) {
  ...
  try {
    ...
    for (const notification of notifications) {
      ...
      const novalnetConfig = getnovalnetConfig(notification)

      await notificationHandler.processNotification({
        notification,
        enableHmacSignature: novalnetConfig.enableHmacSignature,
        ctpProjectConfig,
        logger,
      })
    }

...
The following diagram shows the integration flow of the notification module based on [novalnet Notification webhooks](https://docs.novalnet.com/development-resources/webhooks).
...
You have to register the Notification module `public URL` in the novalnet Customer Area in order to receive notifications.
...
If you want to do the setup manually, please follow the [instructions](https://docs.novalnet.com/development-resources/webhooks#set-up-notifications-in-your-customer-area) described by novalnet to set up notifications in your live Customer Area.

> Note: HMAC verification is enabled by default. You could use "enableHmacSignature: false" field in [novalnet_CONFIG](./HowToRun.md#environment-variable) to disable the verification feature.
...
novalnet sends notifications which look like this:
  ...
  "notificationItems": [
    {
      "NotificationRequestItem": {
        "additionalData": {
          ...
          "metadata.ctProjectKey": "YOUR_COMMERCETOOLS_PROJECT_KEY" // should match a project key in novalnet_INTEGRATION_CONFIG
        },
        "amount": {
          "currency": "EUR",
          "value": 10100
        ...
        "merchantAccountCode": "YOUR_MERCHANT_ACCOUNT", // should match a merchant account in novalnet_INTEGRATION_CONFIG
...
As one notification module instance can receive notifications from multiple novalnet merchant accounts and for multiple commercetools projects (so-called multitenancy), it is necessary to have these accounts and projects configured and pass as environment variable `novalnet_INTEGRATION_CONFIG`. Follow our [how to run guide](./HowToRun.md#environment-variables) for more details.
...
> All mappings can be found in the [novalnet-events.json](./../resources/novalnet-events.json) file.
...
Before you go live please follow the official novalnet [go-live checklist](https://docs.novalnet.com/development-resources/webhooks#test-and-go-live).

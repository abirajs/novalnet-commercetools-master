...
describe('notification controller', () => {
  ...
  it('when novalnetMerchantAccount is not configured, it should log error and return "accepted" status', async () => {
    // prepare:
    const requestMock = {
      method: 'POST',
      url: '/',
    ...
    expect(cause.message).to.equal(
      ...
      'Configuration for novalnetMerchantAccount is not provided. Please update the configuration: "nonExistingMerchantAccount"',
    )
  })
  ...
  it(
    ...
    async () => {
      ...
      const configGetnovalnetConfigSpy = sandbox
        .stub(config, 'getnovalnetConfig')
        .callsFake(() => ({}))
      config.getnovalnetConfig = configGetnovalnetConfigSpy
